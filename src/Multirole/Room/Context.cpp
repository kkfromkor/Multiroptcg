...
std::unique_ptr<YGOPro::STOCMsg> Context::CheckDeck(const YGOPro::Deck& deck) const noexcept
{
	...
	// [OPCG r47] 금지 페어: 금제 파일의 "$pair A B..." 줄(YGOPro::Banlist::Pairs).
	// 코드는 alias 정규화된 베이스(GetTotalCount와 동일 규약, 별쇄 포함 합산).
	// 금제를 고른 방에서만 검사한다(금지·제한과 같은 규제 묶음).
	if(banlist != nullptr)
	{
		for(const auto& pr : banlist->Pairs())
			if(GetTotalCount(pr.first) > 0U && GetTotalCount(pr.second) > 0U)
				return std::make_unique<STOCMsg>(MakeDeckErrorPair(pr.first, pr.second));
	}
	return nullptr;
}
